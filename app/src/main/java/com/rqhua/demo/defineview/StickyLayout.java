...
public class StickyLayout extends LinearLayout {
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                //DOWN 事件停止 scroller
                if (!mScroller.isFinished() && !isSticky() && isContentViewTop(getContentView()) || !mScroller.isFinished() && !isSticky() && mDirection == DIRECTION.UP) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (dy < 0 && Math.abs(dy) > mTouchSlop && mIsControlled && isContentViewTop(getContentView())) {
                    mLastY = y;
                    mIsControlled = false;
                    mReDirect = true;
                    ev.setAction(MotionEvent.ACTION_CANCEL);
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mDirection == DIRECTION.UP) {
                if (isSticky()) {
                    ...
                    contentListFling(distance, duration);
                    mScroller.forceFinished(true);
                } else {
                    scrollTo(0, currY);
                }
            } else {
                if (isContentViewTop(getContentView())) {
    ...
    /**
     * User {@link #isContentViewTop(View)} instead
     *
     * @param recyclerView
     * @return
     */
     ...
     */
    @Deprecated
    private boolean isExpandableListViewTop(ExpandableListView expandableListView) {
        if (expandableListView != null) {
            int firstVisibleItemPosition = expandableListView.getFirstVisiblePosition();
    ...
    private void contentListFling(int distance, int duration) {
        ...
        if (view != null) {
            if (view instanceof RecyclerView) {
                ...
                recyclerView.fling(0, getScrollerVelocity(distance, duration));
            } else if (view instanceof ExpandableListView) {
                ExpandableListView expandableListView = (ExpandableListView) view;
                if (Build.VERSION.SDK_INT > 20) {
                    expandableListView.fling(getScrollerVelocity(distance, duration));
                } else {
